  ...
  <div>
    ...
    <div class="play md-layout">
      <md-card class="md-layout-item md-size-65 md-align-center">
        <md-card-header>
          <div class="md-title md-layout-item md-small-size-100">
            Matchmaking
          </div>
        </md-card-header>
        <md-card-content>
          ...
          >
            <md-radio v-model="powerup" value="no_powerup" class="md-primary"
              >No Power Up</md-radio
            >
            <md-radio
              v-model="powerup"
              value="powerup_powerfist"
              class="md-primary"
              >Power Fist</md-radio
            >
            <md-radio v-model="powerup" value="powerup_dash" class="md-primary"
              >Sprint</md-radio
            >
          </div>
          ...
          <div
            ...
            "
          >
            <md-radio v-model="map" value="classic" class="md-primary"
              >Classic</md-radio
            >
            <md-radio v-model="map" value="obstacles" class="md-primary"
              >Obstacles</md-radio
            >
          </div>
        </md-card-content>
        <md-card-actions>
          <md-button
            v-if="!this.queueJoined"
            @click="joinQueue()"
            class="md-primary"
            :disabled="sending"
            >Join queue</md-button
          >
          <md-button v-else @click="leaveQueue()" class="md-accent"
            >Leave queue</md-button
          >
        </md-card-actions>
      ...
      >
        <div
          class="spectate-size md-layout-item"
                ...
                <div class="md-layout-item md-alignment-center-left">
                  ...
                  >
                    <img
                      ...
                      :src="userAvatar(game.player1Id)"
                    />
                  </md-avatar>
                </div>
                <div
                  class="md-layout-item md-alignment-center-center md-small-hide"
                >
                  <p class="md-headline">
...
export default class Play extends Vue {
  ...
  async created(): Promise<void> {
    ...
    const activeGamesResponsse = await fetch(
      `http://${process.env.VUE_APP_DOMAIN}:${process.env.VUE_APP_NEST_PORT}/game/active`,
      {
        method: "GET",
      }
    );
  ...
  async destroyed() {
    if (this.queueJoined) {
      ...
      if (success) store.commit("setPopupMessage", "You left the queue");
    }
  }
  ...
  async goToGame(id: string) {
    if (globalFunctions.getToken() === "error") {
      store.commit(
        "setPopupMessage",
        "You must be logged in to spectate a game"
      );
      return;
    }
    const gameJwtResponse = await fetch(
      `http://${process.env.VUE_APP_DOMAIN}:${process.env.VUE_APP_NEST_PORT}/game/requestSpectate?id=${id}`,
      {
        method: "GET",
        headers: {
  ...
  async checkQueue(): Promise<void> {
    ...
    const response = await fetch(
      `http://${process.env.VUE_APP_DOMAIN}:${process.env.VUE_APP_NEST_PORT}/matchmaking/poll`,
      {
        method: "GET",
        headers: {
  ...
  async joinQueue(): Promise<void> {
    ...
    const token = document.cookie.split("Token=")[1].split(";")[0];
    const response = await fetch(
      `http://${process.env.VUE_APP_DOMAIN}:${process.env.VUE_APP_NEST_PORT}/matchmaking/join`,
      {
        method: "POST",
        headers: {
          Authorization: "Bearer " + token,
  ...
  async leaveQueue(): Promise<boolean> {
    if (document.cookie.indexOf("Token") === -1) {
      store.commit(
        "setPopupMessage",
        "You must be logged in to leave the queue"
      );
      return false;
    }
    const token = document.cookie.split("Token=")[1].split(";")[0];
    const response = await fetch(
      `http://${process.env.VUE_APP_DOMAIN}:${process.env.VUE_APP_NEST_PORT}/matchmaking/leave`,
      {
        method: "POST",
        headers: {
          Authorization: "Bearer " + token,
